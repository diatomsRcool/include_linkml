...
from include_test.include_schema import NamedThing
            ...
            for fname in files:
                ...
                try:
                    if fname.endswith('.yaml'):
                        nyaml += 1
                        o: NamedThing = yaml_loader.load(full_fname, NamedThing)
                        pyaml += 1
                    elif fname.endswith('.json'):
                        njson += 1
                        o: NamedThing = json_loader.load(full_fname, NamedThing)
                        pjson += 1
                    elif fname.endswith('.ttl'):
                        nttl += 1
                        o: NamedThing = rdf_loader.load(full_fname, NamedThing)
                        pttl += 1
